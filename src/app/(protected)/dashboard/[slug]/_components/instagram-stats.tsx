...
import { useQueryInstagramProfile, useQueryAutomationPosts } from '@/hooks/user-queries'
import { Avatar, AvatarFallback, AvatarImage } from '@/components/ui/avatar'
import { Instagram, Users, Eye, Play, Heart, MessageCircle, Image as ImageIcon } from 'lucide-react'
...
const InstagramStats = () => {
  ...
  const { data: postsData, isLoading: isLoadingPosts } = useQueryAutomationPosts()
  ...
  const posts = postsData?.status === 200 && postsData.data?.data ? postsData.data.data : []
    ...
    return (
      ...
      >
        ...
        <div className="flex-1">
          ...
          <p className="text-white/70 text-sm">Link your account to unlock analytics</p>
        </div>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="font-semibold text-gray-900 dark:text-white">@{profile?.username}</span>
          ...
          <div className="flex gap-3">
            ...
            <div className="relative w-16 h-20 rounded-lg overflow-hidden bg-gray-200 dark:bg-neutral-700 shrink-0">
              {topContent.media_url ? (
                ...
                <img 
                  src={`/api/instagram-proxy?url=${encodeURIComponent(topContent.media_url)}`}
                  alt="Post thumbnail"
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-gray-400 mt-2">
                {topContent.timestamp ? new Date(topContent.timestamp).toLocaleDateString() : ''}
              </p>
            </div>
          </div>
        </div>
