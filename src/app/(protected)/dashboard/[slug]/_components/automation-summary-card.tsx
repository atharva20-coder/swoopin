...
const AutomationSummaryCard = () => {
  ...
  const automationData: AutomationData = useMemo(() => {
    ...
    // Detect platform from flowNodes - check for YouTube-specific subTypes
    const youtubeSubTypes = [
      "YT_COMMENT",
      "YT_REPLY_COMMENT",
      "YT_SELECT_VIDEOS",
      "YT_KEYWORDS",
    ];
    const hasYouTubeNodes = item.flowNodes?.some(
      (node: { subType?: string }) =>
        node.subType && youtubeSubTypes.includes(node.subType),
    );
    const platform = hasYouTubeNodes ? "YouTube" : "Instagram";

    return {
      platform,
      title: item.name || `${platform} Automation`,
      description:
        item.listener?.prompt ||
        `Automated responses for ${platform} comments and DMs`,
      status: item.active ? "completed" : "pending",
      keywords: item.keywords?.map((k: { word: string }) => k.word) || [],
  ...
  const { platform, title, description, status, keywords, createdAt, metrics } =
    automationData;
      ...
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between">
          ...
          <Button
            variant="outline"
            size="sm"
            className="gap-2 dark:border-neutral-700 dark:text-gray-300 dark:hover:bg-neutral-800"
          >
        ...
        <div className="flex items-center gap-3">
          ...
          <Badge
            ...
            className="ml-auto dark:border-neutral-700"
          >
            ...
            <>
              ...
              {keywords.slice(0, 3).map((keyword, idx) => (
                <Badge
                  key={idx}
                  variant="outline"
                  className="text-xs dark:border-neutral-700 dark:text-gray-300"
                >
                  {keyword}
                </Badge>
              ))}
