...
import { Instagram, Facebook, Twitter, Linkedin, Youtube, Mail, MessageCircle, Users, Plus, CheckCircle } from 'lucide-react'
import Link from 'next/link'
import { useParams } from 'next/navigation'

const PLATFORMS = [
  { id: 'instagram', name: 'Instagram', icon: Instagram, color: '#E1306C', gradient: 'from-[#833AB4] via-[#E1306C] to-[#F77737]' },
  { id: 'facebook', name: 'Facebook', icon: Facebook, color: '#1877F2', gradient: 'from-[#1877F2] to-[#42A5F5]' },
  { id: 'twitter', name: 'Twitter/X', icon: Twitter, color: '#1DA1F2', gradient: 'from-gray-900 to-gray-700' },
  { id: 'linkedin', name: 'LinkedIn', icon: Linkedin, color: '#0A66C2', gradient: 'from-[#0A66C2] to-[#0077B5]' },
  { id: 'youtube', name: 'YouTube', icon: Youtube, color: '#FF0000', gradient: 'from-[#FF0000] to-[#CC0000]' },
  { id: 'whatsapp', name: 'WhatsApp', icon: MessageCircle, color: '#25D366', gradient: 'from-[#25D366] to-[#128C7E]' },
  { id: 'gmail', name: 'Gmail', icon: Mail, color: '#EA4335', gradient: 'from-[#EA4335] to-[#FBBC05]' },
]
...
const ConnectedAccounts = () => {
  ...
  const availablePlatforms = PLATFORMS.filter(p => !connectedPlatforms.find(c => c.id === p.id))
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="text-right">
                  <p className="text-lg font-bold text-gray-900 dark:text-white">
                    {platform.profile?.followers ? formatFollowers(platform.profile.followers) : '-'}
                  </p>
