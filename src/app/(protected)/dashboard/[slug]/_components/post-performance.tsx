...
import { useQueryAutomations, useQueryInstagramProfile } from '@/hooks/user-queries'
import { Link2, MessageSquare, MessageCircle, TrendingUp, Sparkles, X } from 'lucide-react'
...
const PostPerformance = () => {
  ...
  const performanceData = useMemo(() => {
    ...
    return automationsData.data.map((automation): Automation => ({
      id: automation.id,
  ...
  const displayStats = useMemo(() => {
    if (selectedAutomation) {
      const engagement = followerCount > 0 
        ? ((selectedAutomation.dms + selectedAutomation.comments) / followerCount * 100).toFixed(2) 
        : '0.00'
      return {
    ...
    const engagementRate = followerCount > 0 ? ((totals.dms + totals.comments) / followerCount * 100).toFixed(2) : '0.00'
  ...
  return (
    <div className="p-6 rounded-2xl bg-white dark:bg-neutral-900 border border-gray-100 dark:border-neutral-800 h-[500px] flex flex-col">
          ...
          <p className="text-sm text-gray-500 dark:text-gray-400">
            {selectedAutomation ? (
              <span className="flex items-center gap-2">
                Showing: <span className="font-medium text-gray-700 dark:text-gray-300">{selectedAutomation.name}</span>
                <button 
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4 shrink-0">
        {[
          { label: 'Link Requests', value: displayStats.linkRequests, icon: Link2, color: 'text-blue-500' },
          { label: 'DMs Generated', value: displayStats.dms, icon: MessageSquare, color: 'text-purple-500' },
      ...
      <div className="flex-1 overflow-hidden">
        <h4 className="text-sm font-medium text-gray-600 dark:text-gray-400 mb-3">
          Top Performers <span className="text-xs text-gray-400">(by engagement)</span>
        </h4>
          ...
          <div className="overflow-y-auto h-[calc(100%-2rem)] pr-2 space-y-2 scrollbar-thin scrollbar-thumb-gray-200 dark:scrollbar-thumb-gray-700">
            {performanceData.map((automation, index) => {
              ...
              return (
                <div 
                  key={automation.id} 
                  onClick={() => setSelectedAutomation(isSelected ? null : automation)}
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm text-gray-900 dark:text-white truncate">{automation.name}</p>
                  ...
                  <div className="text-right shrink-0">
                    <p className="text-sm font-semibold text-gray-900 dark:text-white">{totalEngagement}</p>
                    <p className="text-[10px] text-gray-500">engagement</p>
                  </div>
                  <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${automation.isActive ? 'bg-emerald-500' : 'bg-gray-300'}`} />
                </div>
              )
            })}
          </div>
