...
const MetricsCard = () => {
  ...
  const metrics = useMemo(() => {
    // Data is already validated by Zod in the hook - use directly
    const automations = data?.data ?? [];
    if (automations.length === 0) return { comments: 0, dms: 0, replies: 0 };

    const currentMonthData = automations.filter(
      (item) => new Date(item.createdAt).getMonth() === currentMonth
    );

    return {
      comments: currentMonthData.reduce(
        (sum, item) => sum + (item.listener?.commentCount ?? 0),
        0
      ),
      dms: currentMonthData.reduce(
        (sum, item) => sum + (item.listener?.dmCount ?? 0),
        0
      ),
      replies: currentMonthData.reduce(
        (sum, item) => sum + (item.listener?.commentReply ? 1 : 0),
        0
      ),
    };
      ...
      <div className="p-6 rounded-2xl bg-white dark:bg-neutral-900 border border-gray-100 dark:border-neutral-800 animate-pulse">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="h-24 bg-gray-100 dark:bg-neutral-800 rounded-xl"
            />
          ))}
        </div>
      </div>
  ...
  const platforms = [
    {
      ...
      color: "#E1306C",
      isActive: true,
      stats: `${metrics.dms} DMs · ${metrics.comments} comments`,
    },
