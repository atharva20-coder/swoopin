...
import { MapPin, Users2, TrendingUp, ZoomIn, ZoomOut, RotateCcw, Building2, Layers, AlertCircle } from 'lucide-react'
import {
  ComposableMap,
  Geographies,
  Geography,
...
const LOCATION_COORDINATES: Record<string, { coordinates: [number, number], type: 'country' | 'state' | 'city' }> = {
  // Countries
  'India': { coordinates: [78.9629, 20.5937], type: 'country' },
  'United States': { coordinates: [-95.7129, 37.0902], type: 'country' },
  'United Kingdom': { coordinates: [-3.4360, 55.3781], type: 'country' },
  ...
  'Brazil': { coordinates: [-51.9253, -14.2350], type: 'country' },
  ...
  'Bangladesh': { coordinates: [90.3563, 23.6850], type: 'country' },
  'Nigeria': { coordinates: [8.6753, 9.0820], type: 'country' },
  'Mexico': { coordinates: [-102.5528, 23.6345], type: 'country' },
  'Philippines': { coordinates: [121.7740, 12.8797], type: 'country' },
  ...
  // Indian Cities
  'Mumbai': { coordinates: [72.8777, 19.0760], type: 'city' },
  ...
  'Hyderabad': { coordinates: [78.4867, 17.3850], type: 'city' },
  ...
  // US Cities
  'New York': { coordinates: [-74.0060, 40.7128], type: 'city' },
  ...
  'Berlin': { coordinates: [13.4050, 52.5200], type: 'city' },
  ...
  'West Bengal': { coordinates: [87.8550, 22.9868], type: 'state' },
...
const getMarkerColor = (percentage: number): { fill: string; stroke: string; glow: string } => {
  if (percentage >= 10) return { 
    fill: '#7C3AED', stroke: '#5B21B6', glow: 'rgba(139, 92, 246, 0.6)'
  }
  if (percentage >= 5) return { 
    fill: '#8B5CF6', stroke: '#6D28D9', glow: 'rgba(139, 92, 246, 0.4)'
  }
  if (percentage >= 3) return { 
    fill: '#A78BFA', stroke: '#7C3AED', glow: 'rgba(167, 139, 250, 0.3)'
  }
  return { 
    fill: '#C4B5FD', stroke: '#A78BFA', glow: 'rgba(196, 181, 253, 0.2)'
  }
}

// Age group colors
const ageColors = [
  '#8B5CF6', '#A78BFA', '#6366F1', '#818CF8', '#3B82F6', '#60A5FA', '#93C5FD',
]
...
const AudienceDemographics = () => {
  ...
  const locationData = useMemo<LocationData[]>(() => {
    ...
    return audience.followerDemographics
  ...
  // Age data from API
  const ageData = audience?.reachedDemographics || []
  
  // Gender data from API  
  const genderData = audience?.engagedDemographics || []
        ...
        <div className="lg:col-span-3 relative rounded-xl overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200 dark:from-neutral-800 dark:to-neutral-900 border border-gray-200 dark:border-neutral-700 min-h-[400px]">
          ...
          >
            <ZoomableGroup
              ...
              minZoom={0.8}
              maxZoom={12}
              translateExtent={[[-500, -300], [1500, 700]]}
            >
          ...
          <div className="absolute bottom-3 left-3 flex items-center gap-3 bg-white/95 dark:bg-neutral-900/95 backdrop-blur-sm rounded-lg px-3 py-2 text-xs border border-gray-200 dark:border-neutral-700">
            <span className="text-gray-500 dark:text-gray-400 font-medium">Intensity:</span>
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 rounded-xl bg-gray-50 dark:bg-neutral-800/50 border border-gray-100 dark:border-neutral-700">
            ...
            {ageData.length > 0 ? (
              <div className="space-y-2">
                {ageData.map((item, i) => (
                  <div key={item.dimension} className="flex items-center gap-2">
                    <span className="text-xs text-gray-500 dark:text-gray-400 w-10">{item.dimension}</span>
                    <div className="flex-1 h-5 bg-gray-200 dark:bg-neutral-700 rounded-full overflow-hidden">
                      <div 
                        className="h-full rounded-full flex items-center justify-end pr-2 transition-all duration-500"
                        style={{ 
                          width: `${Math.min((item.percentage / 40) * 100, 100)}%`,
              ...
              <p className="text-xs text-gray-400 text-center py-4">No age data available</p>
            )}
          </div>
            ...
            {genderData.length > 0 ? (
              <>
                ...
                <div className="flex justify-between mt-3">
                  {genderData.map((item, i) => (
                    <div key={item.dimension} className="flex items-center gap-1.5">
                      <div 
              ...
              <p className="text-xs text-gray-400 text-center py-4">No gender data available</p>
            )}
          </div>
          ...
          <div className="p-4 rounded-xl bg-gray-50 dark:bg-neutral-800/50 border border-gray-100 dark:border-neutral-700">
            ...
            {audience?.followerDemographics && audience.followerDemographics.length > 0 ? (
              <div className="space-y-1.5 max-h-40 overflow-y-auto">
                {audience.followerDemographics.slice(0, 8).map((item, i) => (
                  <div 
                    key={item.dimension} 
              ...
              <p className="text-xs text-gray-400 text-center py-4">No location data available</p>
            )}
          </div>
        </div>
      </div>
