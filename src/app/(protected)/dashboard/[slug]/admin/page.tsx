...
export default function AdminDashboardPage({ params }: PageProps) {
  ...
  const conversionRate = stats?.totalUsers
    ? (
        (((stats.proUsers || 0) + (stats.enterpriseUsers || 0)) /
          stats.totalUsers) *
        100
      ).toFixed(1)
    : 0;
    ...
    <div className="py-8 px-6">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <Button
          onClick={fetchStats}
          variant="outline"
          className="gap-2 dark:bg-neutral-900 dark:border-neutral-800 dark:text-white dark:hover:bg-neutral-800"
        >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-200 dark:border-neutral-800 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span
              className={`flex items-center gap-1 text-sm font-medium ${(stats?.userGrowthPercent || 0) >= 0 ? "text-green-500" : "text-red-500"}`}
            >
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-200 dark:border-neutral-800 p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span
              className={`flex items-center gap-1 text-sm font-medium ${(stats?.proGrowthPercent || 0) >= 0 ? "text-green-500" : "text-red-500"}`}
            >
              {(stats?.proGrowthPercent || 0) >= 0 ? (
                <ArrowUp className="w-4 h-4" />
              ) : (
                <ArrowDown className="w-4 h-4" />
              )}
          ...
          <div className="mt-4">
            <MiniChart
              data={stats?.monthlyData?.map((m) => m.proUsers) || []}
              color="#a855f7"
            />
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-200 dark:border-neutral-800 p-6">
          ...
          <div className="mt-4">
            <MiniChart
              data={
                stats?.monthlyData?.map(
                  (m, i, arr) => (m.proUsers / (m.users || 1)) * 100,
                ) || []
              }
              color="#f97316"
            />
            ...
            <Link href={`/dashboard/${slug}/admin/enquiries`}>
              ...
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-3">
                    <Building2
                      className={`w-5 h-5 ${(stats?.pendingEnquiries || 0) > 0 ? "text-amber-600" : "text-gray-500"}`}
                    />
                    <div>
                      <p className="font-medium text-gray-900 dark:text-white">
      ...
      <div className="bg-white dark:bg-neutral-900 rounded-2xl border border-gray-200 dark:border-neutral-800 p-6">
        ...
        <div className="h-64 flex items-end gap-4">
          {(stats?.monthlyData || []).map((monthData, i) => {
            ...
            const height =
              ((stats?.monthlyRevenue?.[i] || 0) / maxRevenue) * 100;
            return (
              <div
                key={monthData.month}
