...
export default function EnquiryDetailPage({ params }: PageProps) {
  ...
  const { slug, id: enquiryId } = React.use(params);
  ...
  const [generatedPaymentUrl, setGeneratedPaymentUrl] = useState<string | null>(
    null,
  );
    ...
    try {
      ...
      if (data.success && data.enquiry) {
        ...
        setFormData({
          ...
          automationsLimit:
            data.enquiry.customAutomationsLimit?.toString() || "",
    ...
    try {
      const response = await fetch(`/api/admin/enquiries/${enquiryId}`, {
        ...
        body: JSON.stringify({
          ...
          customDmsLimit: formData.dmsLimit
            ? parseInt(formData.dmsLimit)
            : null,
          customAutomationsLimit: formData.automationsLimit
            ? parseInt(formData.automationsLimit)
            : null,
          customScheduledLimit: formData.scheduledLimit
            ? parseInt(formData.scheduledLimit)
            : null,
          customAiLimit: formData.aiLimit ? parseInt(formData.aiLimit) : null,
          dealAmount: formData.dealAmount
            ? parseFloat(formData.dealAmount)
            : null,
  ...
  const handleCheckPaymentStatus = async () => {
    ...
    try {
      const response = await fetch(
        `/api/admin/enquiries/${enquiryId}/payment-status`,
      );
      const data = await response.json();
  ...
  const handleActivateEnterprise = async () => {
    ...
    try {
      ...
      if (data.success) {
        toast.success(
          `Enterprise activated for ${formData.subscriptionDays} days!`,
        );
  ...
  const handleDowngrade = async () => {
    if (
      !confirm(
        "Are you sure you want to downgrade this user to FREE? All automations will be paused.",
      )
    ) {
      return;
    }
    ...
    <div className="max-w-4xl mx-auto py-6 px-4">
      ...
      <div className="flex items-center gap-4 mb-6">
        <Button
          variant="ghost"
          size="icon"
          onClick={() => router.push(`/dashboard/${slug}/admin/enquiries`)}
        >
        ...
        <div className="flex items-center gap-2 ml-auto">
          <span
            className={`px-3 py-1 rounded-full text-sm font-medium ${statusColors[enquiry.status]}`}
          >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, dmsLimit: e.target.value })
                }
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, automationsLimit: e.target.value })
                }
                placeholder="e.g. 50"
                className="w-full mt-1 px-3 py-2 rounded-lg border border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-gray-900 dark:text-white"
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, scheduledLimit: e.target.value })
                }
                placeholder="e.g. 100"
                className="w-full mt-1 px-3 py-2 rounded-lg border border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-gray-900 dark:text-white"
              />
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) =>
                  setFormData({ ...formData, aiLimit: e.target.value })
                }
                placeholder="e.g. 200"
                className="w-full mt-1 px-3 py-2 rounded-lg border border-gray-200 dark:border-neutral-700 bg-white dark:bg-neutral-800 text-gray-900 dark:text-white"
              />
              ...
              <div className="mt-4 pt-4 border-t border-gray-200 dark:border-neutral-700">
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-gray-500 flex items-center gap-1">
                    ...
                    Expires:{" "}
                    {new Date(enquiry.paymentLinkExpiresAt).toLocaleString()}
                  </span>
                </div>
                <div className="flex items-center gap-2 mt-2">
                  ...
                  <Button
                    variant="outline"
                    size="sm"
                    onClick={() => copyToClipboard(enquiry.paymentLinkUrl!)}
                  >
                    ...
                    {enquiry.subscriptionEndDate
                      ? (() => {
                          const daysLeft = Math.ceil(
                            (new Date(enquiry.subscriptionEndDate).getTime() -
                              Date.now()) /
                              (1000 * 60 * 60 * 24),
                          );
                          return daysLeft > 0 ? (
              ...
              {enquiry.subscriptionEndDate && (
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-neutral-700">
                  <p className="text-sm text-gray-500 dark:text-neutral-400 mb-3">
                    ⏰ If payment is not received by the due date, the user will
                    be automatically downgraded to FREE and all automations will
                    be paused.
                  </p>
                </div>
              )}
              ...
              {(isEnterprise || enquiry.isActive) && (
                <div className="mt-4 pt-4 border-t border-gray-200 dark:border-neutral-700">
                  ...
                  >
                    {isDowngrading
                      ? "Downgrading..."
                      : "⚠️ Downgrade to FREE (Pause All Automations)"}
                  </Button>
                </div>
              )}
            </div>
