      ...
      try {
        ...
        if (data.status === 200) {
          ...
          setUsage({
            ...
            dmsLimit: data.data.usage.dms.unlimited
              ? -1
              : data.data.usage.dms.limit,
            automations: data.data.usage.automations.used,
            automationsLimit: data.data.usage.automations.unlimited
              ? -1
              : data.data.usage.automations.limit,
            scheduledPosts: data.data.usage.scheduledPosts.used,
            scheduledPostsLimit: data.data.usage.scheduledPosts.unlimited
              ? -1
              : data.data.usage.scheduledPosts.limit,
          });
  ...
  const handleSubscribe = async (plan: Plan) => {
    ...
    try {
      const response = await fetch(
        `/api/payment?plan=${plan}&cycle=${billingCycle}`,
      );
      const data = await response.json();
      if (data.session_url) {
  ...
  const handleEnterpriseSubmit = async () => {
    ...
    try {
      ...
      if (response.ok && data.success) {
        toast.success(
          "Enterprise request submitted! We'll review and get back to you soon.",
        );
    ...
    <div className="max-w-6xl mx-auto py-8 px-4">
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-gray-600 dark:text-gray-400 max-w-2xl mx-auto">
          Choose the perfect plan to grow your Instagram presence. All plans
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <div>
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className="font-medium text-gray-900 dark:text-white">
                {usage.dmsLimit === -1
                  ? `${usage.dmsUsed} / Unlimited`
                  : `${usage.dmsUsed}/${usage.dmsLimit}`}
              </span>
            </div>
              ...
              <div
                ...
                style={{
                  width:
                    usage.dmsLimit === -1
                      ? "20%"
                      : `${usagePercentage(usage.dmsUsed, usage.dmsLimit)}%`,
                }}
              />
            </div>
          </div>
            ...
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className="font-medium text-gray-900 dark:text-white">
                {usage.automationsLimit === -1
                  ? `${usage.automations} / Unlimited`
                  : `${usage.automations}/${usage.automationsLimit}`}
              </span>
            </div>
            <div className="h-2 bg-gray-200 dark:bg-neutral-700 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width:
                    usage.automationsLimit === -1
                      ? "20%"
                      : `${usagePercentage(usage.automations, usage.automationsLimit)}%`,
                }}
              />
            </div>
          </div>
          <div>
            <div className="flex justify-between text-sm mb-1">
              ...
              <span className="font-medium text-gray-900 dark:text-white">
                {usage.scheduledPostsLimit === -1
                  ? `${usage.scheduledPosts} / Unlimited`
                  : `${usage.scheduledPosts}/${usage.scheduledPostsLimit}`}
              </span>
            </div>
            <div className="h-2 bg-gray-200 dark:bg-neutral-700 rounded-full overflow-hidden">
              <div
                ...
                style={{
                  width:
                    usage.scheduledPostsLimit === -1
                      ? "20%"
                      : `${usagePercentage(usage.scheduledPosts, usage.scheduledPostsLimit)}%`,
                }}
              />
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
          <div className="bg-white dark:bg-neutral-900 rounded-2xl p-6 max-w-md w-full">
            ...
            <p className="text-gray-600 dark:text-gray-400 mb-6">
              You&apos;ll be downgraded to the Starter plan at the end of your
              billing period. Your automations beyond the free limit will be
              paused.
            </p>
