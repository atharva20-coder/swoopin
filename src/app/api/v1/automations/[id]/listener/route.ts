...
export async function POST(
  ...
  try {
    ...
    // 6. Save listener (IDOR check inside service)
    const saved = await automationService.saveListener(id, userId, validation.data);
    ...
    if (error instanceof Error) {
      console.error('POST /api/v1/automations/[id]/listener error:', error.message);
    }
