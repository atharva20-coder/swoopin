...
import { SIDEBAR_MENU_GROUPED, SIDEBAR_BOTTOM_ITEMS, ADMIN_SIDEBAR_MENU, MenuCategory } from "@/constants/menu";
    ...
    return (
      <div className="space-y-1">
        {ADMIN_SIDEBAR_MENU.map((item) => {
          ...
          const isActive = isHome ? page === "admin" : page === item.label || page.startsWith(item.label);
          ...
          return (
            ...
            >
              ...
              {!isCollapsed && <span className="text-sm">{item.name || item.label}</span>}
            </Link>
          );
        })}
      </div>
                    ...
                    return (
                      ...
                      >
                        <span className={cn(
                          ...
                          isActive ? "text-purple-600 dark:text-purple-400" : "text-gray-500 dark:text-gray-500 group-hover:text-gray-700 dark:group-hover:text-gray-300"
                        )}>
                          {item.icon}
                        </span>
                        {!isCollapsed && (
                          <span className="text-sm capitalize">{item.name || item.label}</span>
                        )}
                      </Link>
                    );
                  })}
