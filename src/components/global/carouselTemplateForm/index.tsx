...
// REST API helper for creating carousel template
async function createCarouselTemplateApi(
  automationId: string,
  elements: CarouselElement[]
) {
  const res = await fetch(`/api/v1/automations/${automationId}/carousel`, {
    method: "POST",
  ...
  return {
    ...
    data: data.error?.message || "Failed to create carousel template",
  };
}
...
export default function CarouselTemplateForm({
  ...
  const addButton = (elementIndex: number) => {
    ...
    updatedElements[elementIndex].buttons.push({
      type: "WEB_URL",
      title: "",
      url: "",
    });
  ...
  const removeButton = (elementIndex: number, buttonIndex: number) => {
    ...
    updatedElements[elementIndex].buttons = updatedElements[
      elementIndex
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await createCarouselTemplateApi(automationId, elements);
          ...
          <Card key={elementIndex}>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label htmlFor={`title-${elementIndex}`}>
                ...
                <p className="text-xs text-muted-foreground">
                  {element.title.length}/80 characters
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    updateElement(elementIndex, "subtitle", e.target.value)
                  }
                  placeholder="Element subtitle (optional)"
                  maxLength={80}
                />
                <p className="text-xs text-muted-foreground">
                  {element.subtitle?.length || 0}/80 characters
                </p>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <Input
                    ...
                    onChange={(e) =>
                      updateElement(elementIndex, "imageUrl", e.target.value)
                    }
              ...
              <div className="space-y-2">
                <Label htmlFor={`default-action-${elementIndex}`}>
                  Default Action URL
                </Label>
                <Input
                  ...
                  onChange={(e) =>
                    updateElement(elementIndex, "defaultAction", e.target.value)
                  }
                  placeholder="https://example.com"
                />
                <p className="text-xs text-muted-foreground">
                  URL to navigate when the carousel element is clicked
                </p>
              </div>
              ...
              <div className="space-y-4">
                ...
                {element.buttons.map((button, buttonIndex) => (
                  ...
                  >
                    ...
                    <div className="space-y-2">
                      <Label
                        htmlFor={`button-title-${elementIndex}-${buttonIndex}`}
                      >
                    ...
                    <div className="space-y-2">
                      <Label
                        htmlFor={`button-type-${elementIndex}-${buttonIndex}`}
                      >
                        Button Type
                      </Label>
                      <Select
                        ...
                        onValueChange={(value: ButtonType) =>
                          updateButtonType(elementIndex, buttonIndex, value)
                        }
                      >
                        <SelectTrigger
                          id={`button-type-${elementIndex}-${buttonIndex}`}
                        >
                    ...
                    {button.type === "WEB_URL" ? (
                      <div className="space-y-2">
                        <Label
                          htmlFor={`button-url-${elementIndex}-${buttonIndex}`}
                        >
                      ...
                      <div className="space-y-2">
                        <Label
                          htmlFor={`button-payload-${elementIndex}-${buttonIndex}`}
                        >
